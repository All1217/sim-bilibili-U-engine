...
def analyze(isSave=True):
    ...
    for uid, total, night_count in cursor.fetchall():
        total_users += 1
    ...
    conn.close()
    if isSave:
        saveJson("../../Assets/behaviorAnalysis.json", result)
    return result
...
def recommend():
    ...
    resDict = {}
    try:
        res = loadJson("../../Assets/behaviorAnalysis.json")
    ...
    print(f"  互动积极分子建议阈值（大于等于）：THRESHOLDS['active_threshold'] = {active_threshold}  # 覆盖约25%用户")
    ...
    if interval_dist and len(interval_dist) > 0:
        ...
        print(f"  潜水观望者建议阈值（小于等于）：THRESHOLDS['passive_threshold'] = {passive_threshold}  # 当前 {passive_percent}% 用户在此区间")
    resDict['passive_threshold'] = passive_percent
    # 夜猫子阈值
    if night_analysis:
        night_percent = night_analysis.get('night_owl_percentage', 0)
        print(f"  夜猫子判定（凌晨(0-5点)弹幕占比 >= ？）：THRESHOLDS['night_ratio_threshold'] = 0.3  # 当前识别出 {night_percent}% 用户")
        print(f"  夜猫子最小样本数（至少？条弹幕才做判断）：THRESHOLDS['night_min_samples'] = 10      # 至少10条弹幕才判断")
        resDict['night_ratio_threshold'] = night_percent
        resDict['night_min_samples'] = 10
    try:
        print("\n尝试保存建议到JSON……")
        saveJson("../../Assets/recommend.json", resDict)
        print("保存成功！")
        print("\n尝试保存建议到Redis……")
        redisConn = getRedisConn()
        redisConn.hset(BEHAVIOR_THRESHOLD_KEY, mapping=resDict)
        print("保存成功！")
    except FileNotFoundError:
        print("保存失败！")
...
if __name__ == "__main__":
    ...
    # 打印分析结果
    # resultPrint(loadJson('../../Assets/behaviorAnalysis.json'))
